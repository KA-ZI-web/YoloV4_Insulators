...
class DecodeBox(nn.Module):
    ...
    def forward(self, input):
        ...
        # 生成先验框的宽高
        # 确保 scaled_anchors 是 [N, 2] 形状
        anchors = torch.tensor(scaled_anchors, dtype=torch.float32, device='cuda')

        # 直接取宽高
        anchor_w = anchors[:, 0].view(1, -1, 1, 1)  # 调整形状匹配预测张量
        anchor_h = anchors[:, 1].view(1, -1, 1, 1)

        # 计算预测框
        pred_boxes = torch.zeros(prediction[..., :4].shape, dtype=torch.float32, device='cuda')
        pred_boxes[..., 0] = x.data + grid_x  # x 中心
        pred_boxes[..., 1] = y.data + grid_y  # y 中心
